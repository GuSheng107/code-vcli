---
name: code-vcli-ocr
description: 使用 code-vcli CLI 对截图/图片执行本地视觉识别（OCR）与网页 UI 元素解析。当需要把屏幕截图、网页截图、文档图片转成文本或结构化 JSON 时使用。支持普通模式（PP-OCRv6）与 Web 模式（叠加 YOLO UI 检测）。
---

# code-vcli-ocr — VisionCLI 截图转文本

`code-vcli` 是一个面向 AI 编码模型的本地视觉识别 CLI：把一张图片（截图、网页截图、文档扫描件）转成文本，可选输出适合 AI 读取的 JSON。推理完全在本地进行，图片不会上传。

## 前置条件

- 已执行过 `code-vcli init`（首次使用会下载模型，约 1-2 GB）。
- 若返回 `MODEL_NOT_INSTALLED`，则需要先运行 `code-vcli init`。
- 支持 Node.js 22+。
...
| 场景 | 模式 | 命令 |
| --- | --- | --- |
| 网页 / UI 截图（需要定位按钮、输入框、卡片等元素） | `--web` | `code-vcli run ./webpage.png --web --json` |
| 普通文档 / 扫描件 / 纯文字图片 | 默认（普通模式，PP-OCRv6） | `code-vcli run ./image.png --json` |
...
# Web 模式（网页/UI 截图）
code-vcli run ./webpage.png --web

# Web 模式 + JSON
code-vcli run ./webpage.png --web --json
```
...
| 错误码 | 含义 | 处理建议 |
| --- | --- | --- |
| `MODEL_NOT_INSTALLED` | 视觉模型未安装 | 先运行 `code-vcli init` |
...
| `MODEL_RUNTIME_MISSING` | Python 运行时或推理脚本丢失 | 重新运行 `code-vcli init` |
| `MODEL_INITIALIZATION_FAILED` | 模型初始化/下载失败 | 重新运行 `code-vcli init`，检查网络 |
...
| 命令 | 说明 |
| --- | --- |
| `code-vcli init [--yes] [--workspace <path>]` | 初始化视觉模型环境（工作区 + venv + 模型下载） |
| `code-vcli info` | 显示环境信息（Python / 模型 / 状态） |
| `code-vcli update` | 更新到最新版 |
| `code-vcli install [--force]` | 安装到用户目录并加入 PATH |
| `code-vcli version [--check]` | 查看版本 |
| `code-vcli help` | 查看帮助 |